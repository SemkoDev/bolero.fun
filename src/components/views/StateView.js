...
export default class StateView extends Component {
  ...
  render() {
    ...
    if (
      state.iri &&
      state.nelson &&
      state.nelson.status !== "running" &&
      state.iri.status !== "running" &&
      Object.values(state).filter(s => s.status === "error").length > 0
    ) {
      return <ErrorView state={state} />;
    }
    if (
      Object.values(state).filter(s =>
        ["checking", "downloading", "waiting"].includes(s.status)
      ).length
    ) {
    ...
    const isIRISyncronized =
      state.iri.status === "running" &&
      state.iri.info.latestSolidSubtangleMilestoneIndex > 338000 &&
      state.iri.info.latestSolidSubtangleMilestoneIndex ===
        state.iri.info.latestMilestoneIndex;
        ...
        <List divided relaxed>
          <List.Item>
            ...
            <List.Content>
              ...
              <List.Description>
                {state.iri.status === "running" ? "Running" : "Not running"}
              </List.Description>
            </List.Content>
          </List.Item>
          <List.Item>
            ...
            <List.Content>
              ...
              <List.Description>
                {isIRISyncronizedText}&nbsp;{iriMilestones}
              </List.Description>
            </List.Content>
          </List.Item>
          <List.Item>
            ...
            <List.Content>
              ...
              <List.Description>
                {state.iri.status === "running" ? state.iri.info.neighbors : 0}
              </List.Description>
            </List.Content>
          </List.Item>
          <List.Item>
            ...
            <List.Content>
              ...
              <List.Description>
                {state.nelson.status === "running" ? "Running" : "Not running"}
              </List.Description>
            </List.Content>
          </List.Item>
          <List.Item>
            ...
            <List.Content>
              <List.Header>
                Full Node URL for your wallet ({this.state.copiedAddress > 0
                  ? `Copied [${this.state.copiedAddress}]`
                  : "Click to copy"})
              </List.Header>
              <List.Description
                as="a"
                onClick={() => this.copyAddress(localAddress)}
              >
